...
if (!chrome.bananocoinBananojs.getBananoAccountValidationInfo(to_address).valid && (type === "send" || type === "change")) {
  window.close();
}
...
if (type === "send") {
  ...
  document.getElementById("accept").onclick = async function() {
    ...
    chrome.bananocoinBananojs.sendBananoWithdrawalFromSeed(seed, 0, to_address, amount).then(() => {
      window.close();
    }).catch((e) => {
  ...
  document.getElementById("accept").onclick = async function() {
    ...
    chrome.bananocoinBananojs.changeBananoRepresentativeForSeed(seed, 0, to_address).then(() => {
      window.close();
    }).catch((e) => {
      document.getElementById("error-msg").innerText = "Error during send";
      setTimeout(function() {
