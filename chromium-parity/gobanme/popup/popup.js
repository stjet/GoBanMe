...
function store_seed() {
  ...
  chrome.storage.local.set({ "encrypted": JSON.stringify(encrypted), "nonce": JSON.stringify(nonce) });
}
async function get_seed() {
  let password = document.getElementById("password").value;
  document.getElementById("password").value = "";
  let key = chrome.nacl.hash(string_to_uint8(password)).slice(32);
  let nonce = Uint8Array.from(Object.values(JSON.parse((await chrome.storage.local.get("nonce")).nonce)));
  let encrypted = Uint8Array.from(Object.values(JSON.parse((await chrome.storage.local.get("encrypted")).encrypted)));
  try {
    return uint8_to_string(chrome.nacl.secretbox.open(encrypted, nonce, key));
  } catch {
    return 'invalid';
  }
}
