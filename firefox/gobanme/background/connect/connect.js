...
document.getElementById("accept").onclick = async function() {
  ...
  let msg_hashed = uint8_to_hex(browser.nacl.hash(string_to_uint8(msg_og)));
  let priv_key = browser.bananocoinBananojs.getPrivateKey(seed, 0);
  //signed message, in hex form
  let signed_hex = browser.bananocoinBananojs.signHash(priv_key, msg_hashed);
  let address = await browser.bananocoinBananojs.getBananoAccountFromSeed(seed, 0);
  //send
